...
def main():
    ...
    if nuc_type=="DNA":
        for i in seq.keys():
            ...
            dic=digest_iM_for_software(minus_seq, output_conformation,representative_conformation,overlapped, greedy,stem_short,stem_long,loop1_short,loop1_long,loop2_short,loop2_long,loop3_short,loop3_long)
            if dic != False:
                if "form1" in dic.keys():
                    for j in dic["form1"]:
                        out1.write("\t".join([i+"-",str(j[0]),str(j[1]),str(len(j[2])),str(len(j[3])),str(len(j[5])),str(len(j[7]))])+"\t"+"\t".join(j[2:])+"\n")
