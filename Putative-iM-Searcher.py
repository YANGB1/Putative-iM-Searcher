...
def choose_loogest_conformation(select_tmp):
    if len(select_tmp)==1:
        return select_tmp
    loog=0
    all_dic={}
...
def sorted_all_conformation_output_longest_shortest_each_position(greedy,conformation):
    ...
    if greedy==True:
        for i in dic.keys():
            result[i]=choose_loogest_conformation(dic[i])
    else:
        for i in dic.keys():
...
def digest_Overlapped_segment(conformation_each_position):
    ...
    for i in conformation_each_position.keys():
        max_stem_set_tmp=choose_long_stem(conformation_each_position[i])
...
def main():
    parser = argparse.ArgumentParser(description="Welcome to Putative-iM-Searcher. This is to detect putative DNA or RNA putative i-motif forming sequences")
    parser.add_argument("--sequence", help="Show the path of sequence file in fasta format, default=None", type=str)
    parser.add_argument("--overlapped", help="Choose overlapping strategy (type 1) or non-overlapping strategy (type 2), default=2", type=int)
    parser.add_argument("--greedy", help="Choose greedy strategy (type 1) or non-greedy strategy (type 2), default=2", type=int)
    parser.add_argument("--stem_short", help="Set the lower boundary of C-tract length (can not be lower than 2), default=3", type=int)
    parser.add_argument("--stem_long", help="Set the higher boundary of C-tract length, default=100", type=int)
    parser.add_argument("--loop1_short", help="Set the lower boundary of loop1 length (can not be lower than 0, we recommand at least 1), default=1", type=int)
    parser.add_argument("--loop1_long", help="Set the higher boundary of loop1 length, default=12", type=int)
    parser.add_argument("--loop2_short", help="Set the lower boundary of loop2 length (can not be lower than 0,we recommand at least 1), default=1", type=int)
    parser.add_argument("--loop2_long", help="Set the higher boundary of loop2 length, default=12", type=int)
    parser.add_argument("--loop3_short", help="Set the lower boundary of loop3 length (can not be lower than 0,we recommand at least 1), default=1", type=int)
    parser.add_argument("--loop3_long", help="Set the higher boundary of loop3 length, default=12", type=int)
    parser.add_argument("--representative_conformation", help="Set the output of representative conformation (1 for average, 2 for shorter side loop, 3 for both), default=3", type=int)
    parser.add_argument("--output_conformation", help="Show all conformation of all putative i-motifs (1 for True, 2 for False) , default=2", type=int)
    ...
    parser.add_argument("--nuc_type", help="Show the type of nucleic acid, DNA or RNA , default=DNA", type=str)
    ...
    parser.set_defaults(
        ...
        loop3_short=1,
        loop3_long=12,
        representative_conformation=3,
        output_conformation=2,
        output_folder="",
        nuc_type="DNA"
        )
    ...
    try:
        ...
        exit()
    if stem_long<2 or stem_long<stem_short:
        print("Check the stem_long parameter.")
        exit()
    ...
    try:
        ...
        exit()
    if loop1_long<0 or loop1_long<loop1_short:
        print("Check the loop1_long parameter.")
        exit()
    ...
    try:
        ...
        exit()
    if loop2_long<0 or loop2_long<loop2_short:
        print("Check the loop2_long parameter.")
        exit()
    ...
    try:
        ...
        exit()
    if loop3_long<0 or loop3_long<loop3_short:
        print("Check the loop3_long parameter.")
        exit()
    
    representative_conformation=args.representative_conformation
    if representative_conformation not in [1,2,3]:
        print("Check the representative_conformation parameter.")
        exit()
        
    output_conformation=args.output_conformation
    try:
        output_conformation=int(output_conformation)
    except:
        print("Check the output_conformation parameter.")
        exit()
    if output_conformation==1:
        output_conformation=True
    elif output_conformation==2:
        output_conformation=False
    else:
        print("Check the output_conformation parameter.")
        exit()
    ...
    if output_folder!="":
        if not os.path.isdir(output_folder):
            print("Check the output_folder.")
            exit()
    if output_folder=="":
        folder_path, file_name = os.path.split(sequence)
        output_folder=folder_path

    nuc_type=args.nuc_type
    if nuc_type not in ["DNA","RNA"]:
        print("Check the nuc_type.")
        exit()
    ...
    print("output_conformation:",output_conformation)
    print("output_folder:",output_folder)
    print("nuc_type:",nuc_type)
        
    rep1=os.path.join(output_folder,"Putative_iM_Searcher_result_average_conformation.txt")
    rep2=os.path.join(output_folder,"Putative_iM_Searcher_result_side_shorter_conformation.txt")
    all_con_file=os.path.join(output_folder,"Putative_iM_Searcher_result_all_conformation.txt")
    ...
    if representative_conformation==3:
        out1=open(rep1,"w")
        out1.write("\t".join(["ID","start","end","stem_length","loop1_length","loop2_length","loop3_length"])+"\t")
        out1.write("\t".join(["stem1","loop1","stem2","loop2","stem3","loop3","stem4"])+"\n")
        out2=open(rep2,"w")
    ...
    if output_conformation:
        out=open(all_con_file,"w")
        out.write("\t".join(["ID","start","end","stem_length","loop1_length","loop2_length","loop3_length"])+"\t")
        out.write("\t".join(["stem1","loop1","stem2","loop2","stem3","loop3","stem4"])+"\n")
        
        
    for i in seq.keys():
        dic=digest_iM_for_software(seq[i], output_conformation,representative_conformation,overlapped, greedy,stem_short,stem_long,loop1_short,loop1_long,loop2_short,loop2_long,loop3_short,loop3_long)
        if dic != False:
            if "form1" in dic.keys():
                for j in dic["form1"]:
                    if nuc_type=="DNA":
                        ...
                        out1.write("\t".join([i,str(j[0]),str(j[1]),str(len(j[2])),str(len(j[3])),str(len(j[5])),str(len(j[7]))])+"\t"+"\t".join([rnaseg.replace('T', 'U') for rnaseg in j[2:]])+"\n")
            if "form2" in dic.keys():
                for j in dic["form2"]:
                    if nuc_type=="DNA":
                        out2.write("\t".join([i,str(j[0]),str(j[1]),str(len(j[2])),str(len(j[3])),str(len(j[5])),str(len(j[7]))])+"\t"+"\t".join(j[2:])+"\n")
                    else:
                        out2.write("\t".join([i,str(j[0]),str(j[1]),str(len(j[2])),str(len(j[3])),str(len(j[5])),str(len(j[7]))])+"\t"+"\t".join([rnaseg.replace('T', 'U') for rnaseg in j[2:]])+"\n")
                        
            if "all" in dic.keys():
                for j in dic["all"]:
                    if nuc_type=="DNA":
                        out.write("\t".join([i,str(j[0]),str(j[1]),str(len(j[2])),str(len(j[3])),str(len(j[5])),str(len(j[7]))])+"\t"+"\t".join(j[2:])+"\n")
                    else:
                        out.write("\t".join([i,str(j[0]),str(j[1]),str(len(j[2])),str(len(j[3])),str(len(j[5])),str(len(j[7]))])+"\t"+"\t".join([rnaseg.replace('T', 'U') for rnaseg in j[2:]])+"\n")
